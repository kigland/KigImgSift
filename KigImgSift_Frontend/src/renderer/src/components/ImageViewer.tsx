    ...
    return () => {
      ...
      if (preloadRefs.current.size > 20) {
        const currentKeys = new Set(
          imageList.slice(Math.max(0, currentIndex - 5), currentIndex + 5)
        )
  ...
  if (!filename) {
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-100">
        <div className="text-gray-500">没有图片可显示</div>
      </div>
    )
  }
    ...
    <div className="flex-1 flex flex-col bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3">
        <form onSubmit={handleJumpSubmit} className="flex items-center gap-4">
          <label className="text-sm text-gray-600">跳转到:</label>
          <input
            type="number"
            min="1"
            max={totalImages}
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        {loading ? (
          <div className="text-gray-600">正在加载图片...</div>
        ) : imageUrl ? (
          <img
            src={imageUrl}
            alt={filename}
          ...
          <div className="text-red-600">加载图片失败: {filename}</div>
        )}
      </div>
    </div>
  )
