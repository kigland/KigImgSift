...
export class ApiClient {
  static async getImages(): Promise<string[]> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          'ApiClient.getImages: Response not ok. Status:',
          response.status,
          'Body:',
          errorText
        )
  ...
  static async getImageBlob(path: string): Promise<Blob> {
    ...
    try {
      ...
      console.log(
        'ApiClient.getImageBlob: Response headers:',
        Object.fromEntries(response.headers.entries())
      )
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error(
          'ApiClient.getImageBlob: Response not ok. Status:',
          response.status,
          'Body:',
          errorText
        )
      ...
      console.log(
        'ApiClient.getImageBlob: Success, received blob of size:',
        blob.size,
        'type:',
        blob.type
      )
  ...
  static async moveImage(filename: string, categoryId: string): Promise<MoveResponse> {
    ...
    try {
      ...
      console.log(
        'ApiClient.moveImage: Response headers:',
        Object.fromEntries(response.headers.entries())
      )
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error(
          'ApiClient.moveImage: Response not ok. Status:',
          response.status,
          'Body:',
          errorText
        )
        throw new Error(
          `Failed to move image: ${response.status} ${response.statusText} - ${errorText}`
        )
      }
  ...
  static async undoMove(filename: string, fromPath: string, toPath: string, wasCopied?: boolean): Promise<UndoResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error(
          'ApiClient.undoMove: Response not ok. Status:',
          response.status,
          'Body:',
          errorText
        )
        throw new Error(
          `Failed to undo move: ${response.status} ${response.statusText} - ${errorText}`
        )
      }
      ...
      console.error(
        'ApiClient.undoMove: Network or parsing error for filename',
        filename,
        ':',
        error
      )
  ...
  static async getConfig(): Promise<ConfigResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error(
          'ApiClient.getConfig: Response not ok. Status:',
          response.status,
          'Body:',
          errorText
        )
        throw new Error(
          `Failed to get config: ${response.status} ${response.statusText} - ${errorText}`
        )
      }
  ...
  static async updateConfig(config: ConfigResponse): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error(
          'ApiClient.updateConfig: Response not ok. Status:',
          response.status,
          'Body:',
          errorText
        )
        throw new Error(
          `Failed to update config: ${response.status} ${response.statusText} - ${errorText}`
        )
      }
